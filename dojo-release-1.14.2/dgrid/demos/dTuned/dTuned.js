define([
  ...
  var songStore = new RequestMemory({
    target: require.toUrl("./120Ksongs.data.json")
  });
  ...
  artistsList.on("dgrid-select", function(event) {
    ...
    if (row.id === "0") {
      ...
      filteredAlbumList = getFilteredAlbumList(
        gridFilter,
        songStore,
        selectedArtist
      );
